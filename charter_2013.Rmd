...
The data seems to be clustered. Do a response log transform.
```{r}
log_model_sel = lm(formula = log(API13) ~ STYPE + CHARTER + VALID + PCT_AA + PCT_AI + 
    ...
    CW_CSTE + CWS_91 + CW_CSTH + CW_CHSM + CW_SCI + CWM2_28 + 
    CWM2_91 + CWS2_91, data = data_2013)

qqnorm(resid(log_model_sel))
qqline(resid(log_model_sel))
plot(resid(log_model_sel), fitted(log_model_sel))
