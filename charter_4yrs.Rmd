...
We have decided to draw 700 random samples from the entire data set because smaller samples are easier to explain. We then split these samples in half, 350 for training and 350 for testing. With a population size of roughly 7000  valid data, samle size of 350 gives us about 95% confidence of accurately representing the model (https://www.qualtrics.com/blog/calculating-sample-size/).
...
In this particular model, we see that the following predictors being significant  with an significance level of 0.05:
...
Amongst the predictors, the coefficient for parent's education level is the highest, with `r coef(mod_2010)['poly(AVG_ED, 3)1']`, then it is followed by the whether the student is asian or not, with an estimation of `r coef(mod_2010)['poly(PCT_AS, 2)1']`. Please note that those coefficients are related to the first order term, higher order terms actually has negative coefficients. 

Through several iterations, we have found the parent's average education and student's ethnicity consistently shows up when fitting the mode. 

```{r}
pairs(data_2010[, c("AVG_ED", "MEALS", "PCT_AS" , "PCT_AA", "PCT_WH")])
```

`MEALS` corresponds to student's elegibility for free or rduced price meal program. We see that all these predictors are correlated. 


These results simply are correlation and not causation.
