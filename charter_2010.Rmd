...
set.seed(seed)
##########################################
# Randomly select samples that we want to model
#########################################
...
schl_aic = step(schl2010_big_mod, direction = "backward", k = log(n), trace = FALSE, maxit = 100)
...
schl13_aic = step(schl2013_big_mod, direction = "backward", k = log(n), trace = FALSE, maxit = 100)

# check assumptions
bptest(schl13_aic)
shapiro.test(resid(schl13_aic))
...
#                data = schl2010_trn)
 ...
 schl2010_fit = lm(API10  ~ PEN_2 +  PEN_35 +  PCT_AS +  log(PCT_HI)  + PCT_WH     + P_GATE +    P_EL  +   DMOB + PCT_RESP +  poly(AVG_ED, 3) +  ENROLL , data = schl2010_trn )
...
schl2013_fit = lm(API13 ~    PEN_2 +  PEN_35 +   PEN_6  + PEN_78 +  poly(PCT_AS, 2) +  poly(PCT_FI, 1) +   PCT_HI +   poly(PCT_WH, 1)  +   P_EL  +  log(CBMOB) + poly(AVG_ED, 3), data =  schl2013_trn)
