...
This study is to explore the different factors that could affects a student's test score. We want to come up with a model that helps us identify factors that correlate to a student's performance. We want to use the same model and see if these correlations carry through the years. 

Specifically, the data set that we will use comes from https://www.cde.ca.gov/ta/ac/ap/apidatafiles.asp. We will use the "2010 Growth API" data set to train a model. We will use the same model to predict the student's performance in the "2007 Growth API" data set.

The dataset tracks the school's name, district, type (charter or not), student demographics, ranking in the state, class sizes, average parental education levels, and teacher credentials.
...
data_2010 = remove_columns_like(data_2010, c("GRAD", "HSG", "SOME_COL"))
...
We have decided to draw 700 random samples from the entire data set because smaller samples are easier to explain. We then split these samples in half, 350 for training and 350 for testing. With a population size of roughly 7000 valid data, sample size of 350 gives us about 95% confidence with 5% of error to accurately representing the model.
...
Even though this model has some high collinearity issue, it looks pretty promising! We see that ethnicity and parent's education seems to be highly related to student's API scores, we will focus on these predictors to fit our model. 
